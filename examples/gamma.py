...
def plot_polygamma():
    ...
    # for n in [0, 1, 2]:
    #     fp.fline(lambda x: sp.polygamma(n, x), (-3, 3), 1000, ybound=(-20, 20), legend=f"$\\psi_{n}(x)$")
    for x in [-2, -1, 0]:
        fp.axes.axvline(x, color="grey", linestyle="dashed", linewidth=0.75)
...
def plot_fresnel():
    fp = FPlotter()
    fp.fline(lambda x: sp.fresnel(x)[0], (-3, 3), 1000, legend="$S(x)$", auto_clip=False)
    fp.fline(lambda x: sp.fresnel(x)[1], (-3, 3), 1000, legend="$C(x)$", auto_clip=False)
...
def plot_legendre():
    ...
    for n in [0, 1, 2, 3, 4, 5]:
        fp.fline(lambda x: sp.legendre_p(n, x)[0], (-1, 1), 1000, legend=f"$P_{n}(x)$", auto_clip=False)
...
def plot_associated_legendre():
    ...
    for n, m in [(0, 0), (0, 1), (0, 2), (1, 0), (1, 1)]:
        fp.fline(lambda x: sp.assoc_legendre_p(n, m, x)[0], (-1, 1), 1000, legend=f"$P_{n}^{m}(x)$", auto_clip=False)
